<x-main-layout title="E-Complaint Mahasiswa-Tambah">
    <div class="card m-auto col-10">
        ...
        <div class="card-body">
            <form action="{{ route('master.mahasiswa.store') }}" method="POST">
                @csrf
                <div class="row">
                    <div class="col">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-select @error('jurusan_id') is-invalid @enderror" id="jurusan_id"
                                ...
                                @foreach ($jurusans as $jurusan)
                                    <option @selected(old('jurusan_id') === $jurusan->id) value="{{ $jurusan->id }}">
                                        {{ $jurusan->name }}</option>
                                @endforeach
                            </select>
                            @error('jurusan_id')
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-select @error('angkatan') is-invalid @enderror" id="angkatan"
                                ...
                                @foreach ($angkatans as $angkatan)
                                    <option @selected(old('angkatan') === $angkatan) value="{{ $angkatan }}">
                                        {{ $angkatan }}</option>
                                @endforeach
                            </select>
                            @error('angkatan')
